...
function init()
{
	...
	if (window.opera)
		HolidayData = new loadHoliday(element.firstChild.nodeValue) ;
	else
	...
	if (window.opera) 
	{
		for (loop=0; loop<element.length; loop++) 
		{
			...
			{
				element.item(loop).firstChild.nodeValue.match(/[;#] *(.*)\s/) ;
...
function dispMsg(message)
{
	if (isKeitai()) {
		document.getElementById('disptimetable').innerHTML 
				= ("<center><br><font size=+1 color=red>" + message + "</font></center>")  ; 
	} else {
		document.getElementById('disptimetable').innerHTML 
...
function dispDate(message)
{
	if (isKeitai()) {
		document.getElementById('dispdate').innerHTML 
				= ("<div align=right><font color=blue><small>" + message + "</small></font></div>")  ; 
	} else {
...
function dispTimeTable(callByTimer)
{
	...
	else
	{
		...
		if (isKeitai())
		{
			var output = "" ;
			output += "<a name=\"top\"></a>\n" ;
			output += "<hr><center>" + TableList.name + "</center><hr>\n" ;
			
			for (loop=0; loop<TableList.length; loop++) {
				
				output += "<font color=blue>" ;
				output += (((TableList[loop].hour % 24) > 9 ? "" : "&nbsp;") + (TableList[loop].hour % 24) + ":") ;
				output += (((TableList[loop].min > 9 ? "" : "0") + TableList[loop].min)) ;
				output += "&nbsp;</font>" ;
				
				if (State.filter.source == ".*")
					output += TableList[loop].comment ;
				else
					output += TableList[loop].comment.replace(State.filter, "<i>$&</i>") ;
				output += "<br>" ;
				...
				if ((TableList.isRealtime) && (loop<3))
				{
					...
					output += ("<div align=right><font color=red>" + time_last + "</font></div>\n") ;
				}
			}

			output += "<hr></a><a href=\"#top\">▲</a>\n" ;
		}
...
// -------------------------------------------------  
// DateJP()
// 時差情報をもとに日本時間を取得する (AH-K3001のバグ対策)
// -------------------------------------------------  
function DateJP(arg) 
{
	if (arg != undefined)
		return (new Date(arg)) ;
	var UTC = new Date() ;
	var local = (UTC.getTime() + ((UTC.getTimezoneOffset() + 540) * 60000)) ;  // 日本時間(GMTから540分の時差)に設定 
	return (new Date(local)) ;
}

// -------------------------------------------------
// isKeitai()
// UAをチェックしケータイモードの場合 true
// -------------------------------------------------  
function isKeitai()
{
	if (/AH-K300[12]V.*C100/.test(navigator.userAgent)) {
		return true ;
	} else if (/WX3[01]0K.*C100/.test(navigator.userAgent)) {
		return true ;
	} else {
		return false ;
	}
